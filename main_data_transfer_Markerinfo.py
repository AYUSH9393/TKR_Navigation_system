...
def tibia_pointtaker():
    ...
    try:
        ...
        try:
            point_buffer = create_point_status_buffer(
                selectedObject,
                tibia_list,
                femur_list,
                ac_Fetcher_List,
            ...
            if point_buffer:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    ...
                    header = b"POINT_STATUS".ljust(13, b'\x00')
                    s.sendall(header + point_buffer)
                    print("Point status sent to Unity")
        except Exception as e:
            print(f"Error sending point status: {e}")
...
def tibia_verifycuts():
    ...
    if len(tibia_list) >= 4 and not (corner_point[1] == zeroArr).all():
        ...
        buffer = create_verification_buffer(
            selectedObject="tibia",  # or "femur"
            CT_side=CT_side,
            varAngle_combined=varAngle_combined,
            varflex_combined=varflex_combined,
            distance1=Tlc_distance,  # or Dlp_distance
            distance2=Tmc_distance   # or Dmp_distance
        )
        if buffer:
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    s.connect((HOLOLENS_IP, PORT))
                    header = b"VERIFY".ljust(13, b'\x00')  # Ensure header is 13 bytes
                    full_message = header + buffer
                    print(f"Sending total bytes: {len(full_message)}")
                    s.sendall(full_message)  # Send all data at once
            except Exception as e:
                print(f"Error sending verification data: {e}")
        
        #print(tibia_list)

def create_verification_buffer(selectedObject, CT_side, varAngle_combined, varflex_combined, distance1, distance2):
    """Create optimized buffer for verification data"""
    try:
        # Convert values to smaller formats
        object_code = b'T' if selectedObject == "tibia" else b'F'  # 1 byte
        side_code = CT_side.encode('utf-8')  # 1 byte
        
        # Encode the angle strings with fixed lengths
        angle_str = varAngle_combined.encode('utf-8').ljust(16, b'\x00')  # 16 bytes
        flex_str = varflex_combined.encode('utf-8').ljust(16, b'\x00')   # 16 bytes
        
        # Pack distances as shorts
        dist1 = int(distance1)
        dist2 = int(distance2)

        # Create buffer
        buffer = struct.pack(
            '!cc16s16shh',  # Total 38 bytes
            object_code,     # 1 byte
            side_code,       # 1 byte
            angle_str,       # 16 bytes
            flex_str,       # 16 bytes
        ...
        print(f"Sending verification data:")
        print(f"- Object: {object_code}, Side: {side_code}")
        print(f"- Angles: {varAngle_combined}, {varflex_combined}")
        print(f"- Distances: {dist1}, {dist2}")
        print(f"Buffer size: {len(buffer)} bytes")
        return buffer

    except Exception as e:
        print(f"Error creating verification buffer: {e}")
        return None
...
def femur_verifycuts():
    ...
    if len(femur_list) >= 4 and not (corner_point[1] == zeroArr).all():
        ...
        buffer = create_verification_buffer(
            ...
            varAngle_combined=varAngle_combined,
            varflex_combined=varflex_combined,
            distance1=Dlp_distance,  
            distance2=Dmp_distance   
        )
        if buffer:
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
...
def create_marker_status_buffer(corner_point, zeroArr):
    """
    Create a buffer containing marker tracking status
    Returns encoded buffer with marker tracking information
    """
    # Create status dictionary
    marker_status = {
        "pointer": not (corner_point[0] == zeroArr).all(),
        "verification": not (corner_point[1] == zeroArr).all(),
        "tibia": not (corner_point[2] == zeroArr).all(),
        "femur": not (corner_point[3] == zeroArr).all()
    }

    # Convert boolean values to strings
    status_strings = {
        marker: "tracked" if status else "not_tracked" 
        for marker, status in marker_status.items()
    }

    # Encode each status string with fixed length
    pointer_status = status_strings["pointer"].encode('utf-8').ljust(16, b'\x00')
    verification_status = status_strings["verification"].encode('utf-8').ljust(16, b'\x00')
    tibia_status = status_strings["tibia"].encode('utf-8').ljust(16, b'\x00')
    femur_status = status_strings["femur"].encode('utf-8').ljust(16, b'\x00')

    # Pack all statuses into a single buffer
    buffer = struct.pack('16s16s16s16s', 
                        pointer_status,
                        verification_status,
                        tibia_status,
                        femur_status)
...
def create_point_status_buffer(selectedObject, tibia_list, femur_list, ac_Fetcher_List, hip_center_fetcher_list,CT_side):
    """Create buffer for point-taking status"""
    try:
        # Create status byte
        status_byte = 0
        obj_type = b'T' if selectedObject.lower() == "tibia" else b'F'
        side_type = CT_side.encode('utf-8') if CT_side else b'N'  # 'N' for none selected
        
        if selectedObject == "tibia":
            # Pack tibia point status into first 4 bits
            status_byte |= (len(tibia_list) >= 1) << 0  # KC taken
            status_byte |= (len(tibia_list) >= 2) << 1  # TMC taken
            status_byte |= (len(tibia_list) >= 3) << 2  # TLC taken
            status_byte |= (len(tibia_list) >= 4) << 3  # AC taken
            status_byte |= (len(ac_Fetcher_List) == 1) << 4  # Taking MM
            status_byte |= (len(ac_Fetcher_List) == 2) << 5  # Taking LM
        else:  # femur
            # Pack femur point status into first 4 bits
            status_byte |= (femurFirstFrame is not None) << 0  # First frame captured
            status_byte |= (len(femur_list) >= 1) << 1  # HC taken
            status_byte |= (len(femur_list) >= 2) << 2  # FKC taken
            status_byte |= (len(femur_list) >= 3) << 3  # DMP taken
            status_byte |= (len(femur_list) >= 4) << 4  # DLP taken
            
            # Pack hip center points count (bits 5-7)
            hip_points = len(hip_center_fetcher_list)
            status_byte |= (hip_points & 0x07) << 5  # Use 3 bits for point count (0-5)
            
        # Create buffer
        buffer = struct.pack('!ccB',
            obj_type,      # 1 byte for object type ('T' or 'F')
            side_type,     # 1 byte for side ('L', 'R', or 'N')
            status_byte    # 1 byte for status
        )
        
        return buffer
    except Exception as e:
        print(f"Error creating point status buffer: {e}")
        return None

def send_marker_status(corner_point, zeroArr, HOLOLENS_IP, PORT):
    """Send marker tracking status to Unity with optimized buffer"""
    try:
        # Create a single byte for all marker statuses
        status_byte = 0
        status_byte |= (not (corner_point[0] == zeroArr).all()) << 0  # Pointer
        status_byte |= (not (corner_point[1] == zeroArr).all()) << 1  # Verification
        status_byte |= (not (corner_point[2] == zeroArr).all()) << 2  # Tibia
        status_byte |= (not (corner_point[3] == zeroArr).all()) << 3  # Femur

        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.connect((HOLOLENS_IP, PORT))
            s.sendall(b"MARKER_STATUS" + bytes([status_byte]))
    except Exception as e:
        print(f"Error sending marker status: {str(e)}")

def create_rom_buffer(rom_angle, coronal_angle):
    """Create optimized buffer for ROM data"""
    try:
        # Pack angles as 16-bit integers (multiply by 10 to preserve 1 decimal place)
        rom = int(rom_angle * 10)
        coronal = int(float(coronal_angle.split()[0]) * 10)  # Extract number from "X Varus/Valgus"
        
        # Create buffer
        buffer = struct.pack(
            '!HH',  # Total 4 bytes (2 shorts)
            rom,    # 2 bytes for ROM angle
            coronal # 2 bytes for coronal angle
        )
        
        print(f"ROM Buffer - ROM: {rom_angle:.1f}°, Coronal: {coronal/10:.1f}°")
        print(f"Buffer size: {len(buffer)} bytes")
        return buffer

    except Exception as e:
        print(f"Error creating ROM buffer: {e}")
        return None
    
   
# Modify your main loop to send marker tracking information
# Add this inside your while True loop, after corner_point is updated:
...
if __name__ == '__main__':
    ...
    while True:
        ...
        # Create and send marker tracking status buffer
        send_marker_status(corner_point, zeroArr, HOLOLENS_IP, PORT)
        print("Marker status sent to Unity.")
        # try:
        #     marker_status_buffer = create_marker_status_buffer(corner_point, zeroArr)
        #     with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        #         s.connect((HOLOLENS_IP, PORT))
        #         # Add a header to identify this as marker status data
        #         header = b"MARKER_STATUS"
        #         s.sendall(header + marker_status_buffer)
        #     print("Marker status sent to Unity.")
        # except Exception as e:
        #     print(f"Error sending marker status: {str(e)}")
        ...
        if not (corner_point[1] == zeroArr).all():
            if selectedObject == "tibia" and not (corner_point[2] == zeroArr).all():
            ...
            elif selectedObject == "femur" and not (corner_point[3] == zeroArr).all():
        ...
        if isRomOn and corner_point[2] is not None and corner_point[3] is not None:
            ...
            try: 
                ...
                buffer = create_rom_buffer(rom_angle, coronal_rom_angle)
                if buffer:
                    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                        s.connect((HOLOLENS_IP, PORT))
                        header = b"ROM_DATA".ljust(13, b'\x00')  # 13-byte header
                        s.sendall(header + buffer)
                        print("ROM data sent successfully")
            except Exception as e:
