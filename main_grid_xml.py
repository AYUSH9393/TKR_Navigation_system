...
def save_points_to_xml(points, filename="tibia_points.xml"):
    """
    Save collected points to XML file in the specified format
    
    Args:
        points: List of tuples/arrays containing (x,y,z) coordinates
        filename: Output XML filename
    """
    # Create the root element
    root = ET.Element("Data1")
    
    # Add each point as a child element
    for i, point in enumerate(points):
        point_elem = ET.SubElement(root, f"Point_{i}")
        # Format point coordinates with 6 decimal places
        point_str = f"({point[0]:.6f}, {point[1]:.6f}, {point[2]:.6f})"
        point_elem.text = point_str
...
def tibia_pointtaker():
    global tibia_list, ac_Fetcher_List, Point_Taken_label
    global prev_button_a
    global last_button_a_trigger
    ...
    try:
        ...
        if not (corner_point[0] == zeroArr).all() and not (corner_point[2] == zeroArr).all():
            ...
            if (button_allowed) and len(tibia_list) < 25 and CT_side_selected:
                if button_a_pressed:
                    last_button_a_trigger = current_time
                ...
                print(f"Stored point {len(tibia_list)}/25")
                
                # When we have all 25 points, save to XML
                if len(tibia_list) == 25:
                    save_points_to_xml(tibia_list)
                    print("All tibia points collected and saved to XML")
...
def tibia_verifycuts():
    ...
    if len(tibia_list) >= 6 and not (corner_point[1] == zeroArr).all():
        ...
        Tmc_distance = -int(Planes_genrate_LIB.point_plane_distance(tmc_point, vp_normal, center_points[1]))
        Tlc_distance = -int(Planes_genrate_LIB.point_plane_distance(tlc_point, vp_normal, center_points[1]))
...
def femur_pointtaker():
    ...
    try:
        ...
        if (key_pressed or button_allowed) and femurFirstFrame is not None and len(femur_list) <= 6  and CT_side_selected:
            # Update cooldown if triggered by button
            if button_a_pressed:
...
def update_gui():
    ...
    if selectedObject == "tibia":
        ...
        if len(tibia_list) < 25:
            Point_Taken_label.config(text=f"Take Point: {len(tibia_list)+1}/25")
        else:
            Point_Taken_label.config(text="All Points Collected")

        if show_reset_text:
            Point_Taken_label.config(text="Resetting...")
            verification_label.pack_forget()
        # Update values
        if len(tibia_list) >= 26:
...
# class OptimizedCameraApp:
#     def __init__(self):
#         self.data_processor = DataProcessor()
#         self.last_update_time = 0
#         self.update_interval = 1.0 / 30.0  # 30 FPS limit
        
#     def update_loop(self):
#         """Optimized update loop with frame rate limiting"""
#         current_time = time.time()
        
#         # Limit update rate to prevent overwhelming the system
...
#         # Get processed data
#         corner_points, center_points, frames = self.data_processor.send_data()
        
#         # if frames is not None:
#         #     # Extract left frame for display
#         #     left_frame = frames[:, :frames.shape[1]//2]
#         #     self.update_camera_feed(left_frame)
        
#         return corner_points, center_points, frames
    
#     # def update_camera_feed(self, frame):
#     #     """Update camera display (implement your UI update here)"""
#     #     # Your camera feed update logic here
#     #     pass
    
#     def cleanup(self):
#         """Clean up resources"""
#         self.data_processor.release()
