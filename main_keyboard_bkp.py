...
def tibia_verifycuts():
    ...
    global tibia_list,tibiaCount,Sig_Dir,ax,drawPlot,Sig_Dir,var,varAngle_combined,varflex_combined,Tmc_distance,Tlc_distance,CT_side,sagittal_plane,coronal_plane,axial_plane
 
    # Only proceed if tibia_list has at least 6 elements (including the new tuberosity and PCL points)
    if len(tibia_list) >= 6 and not (corner_point[1] == zeroArr).all():
        ...
        mechanical_axis_vector = mechanical_axis_vector / np.linalg.norm(mechanical_axis_vector)  # Normalize
        
        # Generate sagittal plane using tuberosity and PCL points, passing through knee center
        if tibiaCount is None:
            # Create vector from tuberosity to PCL
            tuberosity_to_pcl = pcl_point - tuberosity_point
            tuberosity_to_pcl = tuberosity_to_pcl / np.linalg.norm(tuberosity_to_pcl)  # Normalize
                
                # Create sagittal plane normal by cross product of mechanical axis and tuberosity-PCL vector
        ...
        print("varflex",varflex)
        #Planes_genrate_LIB.reset_distance_history()
        ...
        if varAngle < 0:
            varAngle_name = "Var" if CT_side == "L" else "Val"
        else:
            varAngle_name = "Val" if CT_side == "L" else "Var"
        ...
        # print(f"Sagittal plane normal: {sagittal_normal}")
        # print(f"Coronal plane normal: {coronal_Dir}")
        # print(f"Axial plane normal: {mechanical_axis_vector}")
        # print(f"All planes pass through knee center: {kc_point}")
        # print(f"Varus/Valgus: {varAngle_combined}")
        # print(f"Anterior/Posterior: {varflex_combined}")
        # print(f"TMC distance: {Tmc_distance}")
        # print(f"TLC distance: {Tlc_distance}")
...
def femur_pointtaker():
    ...
    try:
        ...
        if (key_pressed) and femurFirstFrame is not None and len(femur_list) <= 6  and CT_side_selected:
            # Update cooldown if triggered by button
            #if button_a_pressed:
...
def femur_verifycuts():
    ...
    global sagittal_plane, axial_plane, coronal_plane
    ...
    if len(femur_list) >= 6 and not (corner_point[1] == zeroArr).all():
        ...
        # cv2.putText(left, f"medial: {Dmp_distance:.0f} mm", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
        # cv2.putText(left, f"lateral: {Dlp_distance:.0f} mm", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
...
def create_gui():
    ...
    global verification_label, coronal_label, sagittal_label, medial_label, lateral_label, Point_Taken_label,rom_label,screen_width,screen_height,coronal_rom_angle_label
    ...
    rom_label = tk.Label(right_frame, 
                        ...
                        font=("Helvetica", 20),
                        anchor="w",
                        width=Rom_label_width,
                        height=label_height,
                        bg='#314d79',
                        fg='white')
    rom_label.pack(pady=5, padx=5, fill="x")
    
    coronal_rom_angle_label = tk.Label(right_frame, 
                        text="ROM: N/A", 
                        font=("Helvetica", 20),
                        anchor="w",
                        width=Rom_label_width,
                        ...
                        bg='#314d79',
                        fg='white')
    coronal_rom_angle_label.pack(pady=5, padx=5, fill="x")
...
def update_gui():
    ...
    if selectedObject == "tibia":
        ...
        coronal_rom_angle_label.pack_forget()
        if len(tibia_list) == 0:
            Point_Taken_label.pack()
            ...
            Point_Taken_label.config(text="Take Point: Tibia tuberosity")
            verification_label.pack_forget()
        elif len(tibia_list) == 4:
            Point_Taken_label.config(text="Take Point: PCL")
            verification_label.pack_forget()
        elif len(tibia_list) == 5:
        ...
        if len(tibia_list) >= 6:
            ...
            medial_label.config(text=f"Medial: {Tmc_distance +6:.0f} mm")
            lateral_label.config(text=f"Lateral: {Tlc_distance +6:.0f} mm")
        ...
        if len(femur_list) == 0 and femurFirstFrame is not None:
            ...
            Point_Taken_label.config(text="Take Point: Femur Surgical Medial Condyle")
            verification_label.pack_forget()
            pointer_label.pack()
        elif len(femur_list) == 5:
            Point_Taken_label.config(text="Take Point: Femur Surgical Lateral Condyle")
            verification_label.pack_forget()
            pointer_label.pack()
        elif len(femur_list) == 6:
        ...
        if len(femur_list) >= 6:
            ...
            medial_label.config(text=f"Medial: {Dmp_distance + 6:.0f} mm")
            lateral_label.config(text=f"Lateral: {Dlp_distance + 6:.0f} mm")
            pointer_label.pack_forget()
            verification_label.pack()

    if len(tibia_list) >= 6 and len(femur_list) >= 6 and isRomOn:
        # Show ROM label
        rom_label.pack()
        coronal_rom_angle_label.pack()
        ...
        if len(tibia_list) >= 6 and len(femur_list) >= 6:
            rom_label.config(text=f"ROM: {int(rom_angle)}°")
            coronal_rom_angle_label.config(text=f"Coronal ROM: {str(coronal_rom_angle)}°")
        else:
...
if __name__ == '__main__':
    ...
    #pointer_corner_points = np.array(np.load('pv_corner_points_10_7_1.npy')) # Load the captured points 5X3X4 
    #print("pointer_corner_points:",pointer_corner_points)
    ...
    #app = OptimizedCameraApp()

    #root.protocol("WM_DELETE_WINDOW", root.quit)
    while True:
        # Process pygame events
        #data_instance_1.start_threads()
        # pygame.event.pump()  # Poll joystick events
        ...
        if ret:
            ...
            #corner_point, center_points ,is_marker_detected = app.update_loop()
        ...
        # if not (corner_point[2] == zeroArr).all() and not (corner_point[1] == zeroArr).all():
        #     # Calculate distance between tibia and femur markers
        #     marker_distance = np.linalg.norm(center_points[2] - center_points[1])
            
        #     # Calculate angle between markers
        #     # Create vectors for tibia and femur markers
        #     tibia_vector = corner_point[2][1] - corner_point[2][0]  # Using first two corners
        #     femur_vector = corner_point[1][1] - corner_point[1][0]
            
        #     # Normalize vectors
        #     tibia_vector = tibia_vector / np.linalg.norm(tibia_vector)
        #     femur_vector = femur_vector / np.linalg.norm(femur_vector)
            
        #     # Calculate angle between vectors
        #     dot_product = np.dot(tibia_vector, femur_vector)
        #     angle = np.arccos(np.clip(dot_product, -1.0, 1.0))
        #     angle_degrees = np.degrees(angle)
            
        #     # Display both distance and angle on the camera feed
        #     cv2.putText(
        #         left, 
        #         f"Marker Distance: {marker_distance:.1f} mm", 
        #         (10, 30), 
        ...
        #     cv2.putText(
        #         left, 
        #         f"Marker Angle: {angle_degrees:.1f} deg", 
        #         (10, 70), 
        #         cv2.FONT_HERSHEY_SIMPLEX, 
        #         1, 
        #         (0, 255, 0),  # Green color
        ...
        if key == ord('w')  and selectedObject == "femur" and OnlyAtFirstTime and not (corner_point[1] == zeroArr).all() and CT_side_selected: 
            femur_refernce = getRelativeToMarker(corner_point[3],corner_point[1])
        ...
        if trigger_pressed and (current_time - last_trigger_time >= cooldown_duration):
            ...
            if selectedObject == "femur" and len(femur_list) == 1:
                ...
                relative_femur_points_list = []
                OnlyAtFirstTime = True
            elif selectedObject == "femur" and len(femur_list) > 0 and len(femur_list) < 6 and is_taking_Hip_Center == False:
            ...
            #tibia point removal    
            if selectedObject == "tibia" and len(tibia_list) > 0 and len(tibia_list) < 6 and len(ac_Fetcher_List) == 0:
        ...
        if isRomOn and corner_point[2] is not None and corner_point[3] is not None:
            global rom_angle,coronal_rom_angle
            try: 
                ...
                coronal_rom_angle = Angle(New_femur_hip, New_femur_knee,
                            New_tibia_knee, New_tibia_ankle,sagittal_normal_femur,CT_side)[0]
        ...
        # In your main loop, add this after calculating center_points:
        
        rightSideTextXPos = 430
        rightSideTextYPos = 80
        #Display Instructions
        if selectedObject == "femur":
            cv2.putText(left,f"Femur", (rightSideTextXPos, rightSideTextYPos-50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
            if femurFirstFrame is None:
                cv2.putText(left,f"Capture First frame of femur",(rightSideTextXPos-10,rightSideTextYPos),cv2.FONT_HERSHEY_SIMPLEX,0.7,(255,0,0),2)
            #Femur points
            if len(femur_list) == 0 and femurFirstFrame is not None:
                cv2.putText(left,f"Femur Hip Center Points :{len(hip_center_fetcher_list)+1}", (rightSideTextXPos, rightSideTextYPos), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
            if len(femur_list) == 1:
                cv2.putText(left,f"Femur Knee Center", (rightSideTextXPos, rightSideTextYPos), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
            if len(femur_list) == 2:
                cv2.putText(left,f"Femur Distal Medial Condyle", (rightSideTextXPos, rightSideTextYPos), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
            if len(femur_list) == 3:
                cv2.putText(left,f"Femur Distal Lateral Condyle", (rightSideTextXPos, rightSideTextYPos), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
            if len(femur_list) == 3:
                cv2.putText(left,f"SME", (rightSideTextXPos, rightSideTextYPos), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
            if len(femur_list) == 3:
                cv2.putText(left,f"SLE", (rightSideTextXPos, rightSideTextYPos), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
            if len(femur_list) == 4:
                cv2.putText(left,f"Femur Points Taken", (rightSideTextXPos, rightSideTextYPos), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)


        elif selectedObject == "tibia":
            cv2.putText(left,f"Tibia", (rightSideTextXPos, rightSideTextYPos-50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
            #Tibia points
            if len(tibia_list) == 0:
                cv2.putText(left,f"Tibia Knee Center", (rightSideTextXPos, rightSideTextYPos), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
            if len(tibia_list) == 1:
                cv2.putText(left,f"Tibia Medial Condyle", (rightSideTextXPos, rightSideTextYPos), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
            if len(tibia_list) == 2:
                cv2.putText(left,f"Tibia Lateral Condyle", (rightSideTextXPos, rightSideTextYPos), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
            if len(tibia_list) == 3:
                cv2.putText(left,f"Tuberosity", (rightSideTextXPos, rightSideTextYPos), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
            if len(tibia_list) == 4:
                cv2.putText(left,f"PCL", (rightSideTextXPos, rightSideTextYPos), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
            if len(tibia_list) == 5:
                if len(ac_Fetcher_List) == 0:
                    cv2.putText(left,f"Medial Malleolus", (rightSideTextXPos, rightSideTextYPos), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
                if len(ac_Fetcher_List) == 1:
                    cv2.putText(left,f"Lateral Malleolus", (rightSideTextXPos, rightSideTextYPos), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
            if len(tibia_list) == 6:
                cv2.putText(left,f"Tibia Points Taken", (rightSideTextXPos, rightSideTextYPos), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)


        if CT_side == "L":
            cv2.putText(left,f"L", (rightSideTextXPos+80, rightSideTextYPos - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
        elif CT_side == "R":
            cv2.putText(left,f"R", (rightSideTextXPos+80, rightSideTextYPos - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)

        
        #root.update()
        cv2.imshow( "Camera Frame left",left)
        #cv2.imshow( "Camera Frame left",right)
