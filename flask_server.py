...
def get_status():
    try:
        # Ensure all marker status values are JSON serializable
        marker_status_safe = {
            'calibrated': bool(app_state.marker_status.get('calibrated', False)),
            'markers': {}
        }
        
        for marker_id, marker_data in app_state.marker_status['markers'].items():
            marker_status_safe['markers'][int(marker_id)] = {
                'name': str(marker_data.get('name', '')),
                'valid': bool(marker_data.get('valid', False)),
                'size': float(marker_data.get('size', 0)),
                'detected': bool(marker_data.get('detected', False))
            }
        
        return jsonify({
            'selected_object': str(app_state.selected_object) if app_state.selected_object else None,
            'ct_side': str(app_state.ct_side) if app_state.ct_side else None,
            'is_processing': bool(app_state.is_processing),
            'points_captured': {
                'femur': int(app_state.points_captured['femur']),
                'tibia': int(app_state.points_captured['tibia'])
            },
            'data_ready': bool(app_state.data_ready),
            'fps': float(getattr(app_state, 'fps', 0)),
            'total_frames': int(app_state.frame_count),
            'current_point': str(getattr(app_state, 'current_point_message', "-")),
            'marker_status': marker_status_safe
        })
...
def video_feed():
    """Optimized MJPEG streaming route"""
...
def generate_frames():
    """Ultra-optimized generator with frame skipping"""
    encode_param = [int(cv2.IMWRITE_JPEG_QUALITY), 80]  # Slightly lower quality for speed
    frame_header = b'--frame\r\nContent-Type: image/jpeg\r\n\r\n'
    frame_footer = b'\r\n'
    
    while True:
        try:
            # Always get the latest frame, skip old ones
            frame = None
            while not app_state.frame_queue.empty():
                try:
            ...
            if frame.size > 0:
                ...
                if success:
                    yield frame_header + buffer.tobytes() + frame_footer
...
# Alternative: Use turbojpeg for 2-5x faster encoding (requires turbojpeg library)
try:
    from turbojpeg import TurboJPEG
    jpeg = TurboJPEG()
    
    def generate_frames_turbojpeg():
        """Fastest encoding with turbojpeg"""
        frame_header = b'--frame\r\nContent-Type: image/jpeg\r\n\r\n'
        frame_footer = b'\r\n'
        ...
        while True:
            try:
                ...
                if frame.size > 0:
                    # TurboJPEG encoding (much faster than cv2.imencode)
                    buffer = jpeg.encode(frame, quality=85)
                    yield frame_header + buffer + frame_footer
    ...
    print("[INFO] TurboJPEG not available. Install with: pip install PyTurboJPEG")
...
def send_command():
    try:
        ...
        if command == 'select_femur':
            ...
            from main_keyboard import update_next_femur_point
...
def update_marker_status(calibrated, markers_data):
    ...
    try:
        ...
        app_state.marker_status['calibrated'] = bool(calibrated)
        ...
        for marker_id, status in markers_data.items():
            if marker_id in app_state.marker_status['markers']:
                ...
                cleaned_status = {
                    ...
                    'size': float(status.get('size', 0.0))
                }
                app_state.marker_status['markers'][marker_id].update(cleaned_status)
